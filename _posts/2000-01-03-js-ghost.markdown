...
If you want a challenge, make it so that one specific key changes the left eyeball color, and one specific key changes the right eyeball color.

For example, if you press "r", the right eye color should change. If you press "l", the left eye color should change. 

Here's a bit of code to help get you started:

```js
document.addEventListener('keypress', (event) => {
  var keyName = event.key;
  if (keyName === "t") {
    alert("You pressed the letter t!")
  } else if (keyName === "g") {

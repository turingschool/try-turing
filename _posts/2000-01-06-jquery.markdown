...
Play around with the following example. <br><br>

<li>Remove the line of JavaScript.</li>
<li>Change the replace text to something else.</li>
<li>Change the `h1` selector to `.important-header`</li>
<li>Add the following line of code: `$('h1').css('backgroundColor', 'red');`</li>
</div>
...
<p>We're using a jQuery method called `toggleClass()`. When the user clicks on the button, it either adds or remove the class `upside-down` depending on whether or not it was already there.</p>

<li>Can you create some additional classes and toggle them?</li>
<li>Can you change the text?</li>
<li>Try out the following methods:</li>
  - `toggle()`
  - `slideToggle()`
  - `fadeToggle()`
</div>

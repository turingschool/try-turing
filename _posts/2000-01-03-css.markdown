...
The following rule will only be in effect when a user is actively hovering over a `<a>` element.
...
Right now, we know how to style _all_ of the paragraphs or _all_ of the links on a page. But what if we wanted to style just one?
...
Sweet, right? We have [quite a few transformations at our disposal](https://developer.mozilla.org/en-US/docs/Web/CSS/transform), but we'll cover just a subset today:
...
<p>Use the div from above to create a transition that transitions the color, size, and position of the div upon a mouse hover over a 4 second duration.</p>
</div>  

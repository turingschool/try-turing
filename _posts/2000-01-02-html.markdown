...
Of all of the major technologies used on the web—on either the front- or the back-end—HTML is the oldest. In the beginning, the web was just a bunch of HTML documents that you wrote by hand. They had these cool things called hyperlinks that would allow you to click on a word on one page and be taken to another page.
...
We use HTML tags to mark up text to show it's semantic meaning. The browser uses these tags to build up our document. _Most_ tags have an opening and closing tag, but a few do not. Images defined using the `<img>` tag do not have a closing tag for instance.
...
Our `<img>` tag needs an extra little piece of information. Our browser is more than happy to load up an image, but it'd be helpful if we told it where that image is located. That's where the `src` attribute comes in.
...
<aside>
  ...
  <p>
    You may hear me use the term "tag" and "element" interchangeably. The reason for this is that they're closely related, with just a subtle difference. A tag is the thing we write when we're creating an HTML document. The browser then reads and parses our document. Elements are the end result of what's rendered in the browser based on the tags we write.
  </p>
</aside>
...
<div class="try-it">
<h2>Try It: Using Inline Elements</h2>
...
There are many more HTML tags. But, the ones we learned today are enough to get us started and the raw materials that we'll need to build a game by the end of the day. As the day goes on, I'll slip in some additional tags where it's appropriate, but their meaning and significance should be clear in the context that they're presented.
